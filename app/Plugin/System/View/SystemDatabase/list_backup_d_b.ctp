  ...
  <table cellpadding='0' cellspacing='0' class='table table-striped table-bordered'>
    ...
    foreach ($dataList as $file => $name):
      ...
      <tr>
        <td><?= $name ?></td>
        <td class='actions'>
          <?= $this->Form->postLink(__('Revert'), Router::url(array('plugin' => 'System', 'controller' => 'SystemDatabase', 'action' => 'rollbackBackupDB', $file)), array('class' => 'btn btn-default btn-xs btn-delete'), __('NOTE: PLEASE BACKUP BEFORE UPDATE.'."\n".' Are you sure you want to revert the snapshot at "%s"?', $name)) ?>
