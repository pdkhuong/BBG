...
<?php
  ...
  ?> 
  <?= $this->Form->input('role_id', array('label' => 'Role', 'options' => $dataRoleId, 'empty' => 'Please select one')) ?>
  <?= $this->Form->input('account_id', array('label' => 'Account', 'options' => $dataAccountId, 'empty' => 'Please select one')) ?>
