<h3>
  <? if (isset($this->data['ProductOrder']['id']) && $this->data['ProductOrder']['id'] > 0): ?>
    <?= __('Edit Product Order') ?>: #<?= $this->data['ProductOrder']['id'] ?>
  <? else: ?>
...
<?php
  echo $this->Form->create('ProductOrder', array(
    'novalidate' => true,
    'inputDefaults' => array(
      'div' => 'form-group',
      'label' => array(
  ...
  <div class="form-group required <?php if($isErrorDeliveryDate) echo 'has-error error' ?>">
    ...
    <div class="col col-md-3">
      <div class='input-group date _datetime_picker' data-date-format="YYYY-MM-DD">
        <input value="<?php echo isset($this->data['ProductOrder']['delivery_date']) && $this->data['ProductOrder']['delivery_date'] ? reformatDate($this->data['ProductOrder']['delivery_date']) : '' ?>" name="data[ProductOrder][delivery_date]" type='text' class="form-control" readonly="readonly"/>
        ...
        <?php foreach($addedProgress as $progressId => $progress):?>
          <tr id="_row_<?php echo $progressId?>">
            <td>
              ...
              <?php echo $this->Form->input("ProductOrderProgress.{$progressId}.order", array('type' => 'hidden', 'value' => $progress['order']));?>
            </td>
            <td>
              <?php echo $progress['name']?>
              <?php echo $this->Form->input("ProductOrderProgress.{$progressId}.name", array('type' => 'hidden', 'value' => $progress['name']));?>
            </td>
            <td>
              <?php echo $progress['location']?>
              <?php echo $this->Form->input("ProductOrderProgress.{$progressId}.location", array('type' => 'hidden', 'value' => $progress['location']));?>
            </td>
            <td>
              <?php echo $progress['description']?>
              <?php echo $this->Form->input("ProductOrderProgress.{$progressId}.description", array('type' => 'hidden', 'value' => $progress['description']));?>
            </td>
            <td>
...
<script id="tableRowTemplateProductOrder" type="text/x-jQuery-tmpl">
  <tr id="_row_${id}">
    <td>
      <span class="_progress_order">${order}</span>
      <?php echo $this->Form->input('ProductOrderProgress.${id}.order', array('type' => 'hidden', 'value' => '${order}'));?>
    </td>
    <td>
      ${progress_name}
      <?php echo $this->Form->input('ProductOrderProgress.${id}.name', array('type' => 'hidden', 'value' => '${progress_name}'));?>
    </td>
    <td>
      ${progress_location}
      <?php echo $this->Form->input('ProductOrderProgress.${id}.location', array('type' => 'hidden', 'value' => '${progress_location}'));?>
    </td>
    <td>
      ${progress_description}
      <?php echo $this->Form->input('ProductOrderProgress.${id}.description', array('type' => 'hidden', 'value' => '${progress_description}'));?>
    </td>
    <td>
